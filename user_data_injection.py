...
lastName = ['김', '김','김', '김', '김', '김', '김', '김', '김', '김', '이', '이', '이', '이', '이', '이', '이', '이', '이',
            ...
            '윤', '오','배', '엄', '양', '신', '백', '곽', '김', '김', '김', '김', '이', '이', '이', '이', '이', '이', '이'] 
firstName = ['동현','강현','찬형','형석','현석','하연','다혜','다현','원재','주현','상현','도훈','현진','성현','석현','다인',
             ...
             '태원','한별','현철','영무','규호','재훈','선준','정재','현석','종아','형우','현도','홍섭','도헌','필현','순규',
             '의성','춘진','혜원','대송','제환','병욱','태연','장연','호영','영진','현재','세형','상욱','현수','철희','수인']
...
for j in range (0,5000):
    ...
    # ID / PWD / name / gender / birth / email / phone / height / weight / location

    ID = randUpWordNum(random.randrange(4,6),random.randrange(2,4))
    PWD = randUpWordNum(random.randrange(4,6),random.randrange(2,4))
    ...
    birth = randtimestamp(xtime, 14600)
    email = ID + emailAddr[random.randrange(0, len(emailAddr))]
    phone = '010-' + randUpWordNum(0,4) + '-' + randUpWordNum(0,4)
    height = round(float(random.randrange(1500, 1900)) * 0.1, 1)
    weight = round((height - 100) * random.randrange(8, 12) * 0.1)

    sqlInsert = "insert into user(ID, PWD, name, gender, birth, email, phone, height, weight) values(%s, HEX(AES_ENCRYPT(%s,'lifesports')),%s, %s, %s, %s,%s,%s,%s);"
    insertTuple = (ID, PWD, name, gender, birth, email,phone,height,weight)
